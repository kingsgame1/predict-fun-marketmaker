...
- `MM_SAFE_MODE_PAUSE_MS`：安全档触发后短暂停挂时间（毫秒）。
- `MM_SAFE_MODE_CANCEL_THRESHOLD_MULT`：安全档全局撤单阈值倍数。
...
- `MM_WS_HEALTH_FORCE_ONLY_FAR`：WS 健康度较差时强制仅挂远层。
- `MM_WS_HEALTH_ONLY_FAR_LAYERS`：WS 健康度较差时仅挂最远层数。
- `MM_WS_HEALTH_MIN_INTERVAL_MULT_MAX`：WS 健康度最差时下单间隔倍数上限。
- `MM_WS_HEALTH_SIZE_SCALE_MIN`：WS 健康度最差时挂单份额缩放下限（0-1）。
...
- `MM_WS_HEALTH_LAYER_COUNT_CAP`：WS 健康度较差时分层数量上限。
...
- `MM_WS_HEALTH_ULTRA_SAFE_ENABLED`：WS 健康度较差时启用“极限稳健模板”。
- `MM_WS_HEALTH_ULTRA_SAFE_SIDE`：极限稳健单侧方向（BUY/SELL/NONE）。
- `MM_WS_HEALTH_ULTRA_SAFE_MODE`：极限稳健单侧模式（REMOTE/NORMAL）。
- `MM_WS_HEALTH_ULTRA_SAFE_OFFSET_BPS`：极限稳健额外偏移（bps）。
- `MM_WS_HEALTH_ULTRA_SAFE_FAR_LAYERS`：极限稳健仅挂最远层数。
- `MM_WS_HEALTH_ULTRA_SAFE_SIZE_SCALE`：极限稳健挂单份额缩放（0-1）。
...
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_MIN_RATIO`：渐进恢复阶段最低保守比例（0-1，越高越保守）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_STEPS`：渐进恢复的阶段数（步数越多越平滑）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SIZE_SCALE_MIN`：急撤恢复期最小挂单规模缩放（0-1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_LAYER_CAP_MIN`：急撤恢复期最小挂单层数上限（>=1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_INTERVAL_MULT_MAX`：急撤恢复期下单间隔最大放大倍数（>=1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE`：急撤恢复期单边挂单方向（BUY/SELL/NONE）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE_MODE`：急撤恢复期单边挂单模式（REMOTE/NORMAL）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE_EXIT_PROGRESS`：恢复期进度超过此值后退出单边挂单（0-1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_DEPTH_MULT`：急撤恢复期顶层深度阈值乘数（>=1，越大越保守）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_VOLATILITY_MULT_MIN`：急撤恢复期波动阈值最低倍率（0-1，越低越敏感）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SPREAD_ADD`：急撤恢复期额外价差下限（例如 0.002 = 0.2%）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_ICEBERG_RATIO`：急撤恢复期冰山比例（0-1，越低越保守）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_CANCEL_CONFIRM_MULT_MIN`：急撤恢复期撤单确认时间最小倍率（>=1，越大越慢）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_MAX_ORDERS_MULT_MIN`：急撤恢复期最大挂单数量缩放下限（0-1，越低越保守）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_REPRICE_CONFIRM_MULT_MIN`：急撤恢复期重报价确认时间最小倍率（>=1，越大越慢）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_MAX_NOTIONAL_MULT_MIN`：急撤恢复期单市场挂单总名义额缩放下限（0-1，越低越保守）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_FAR_LAYERS_MIN`：急撤恢复期最低挂远层数量（>=1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_CANCEL_INTERVAL_MULT_MAX`：急撤恢复期撤单间隔最大放大倍数（>=1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_FAR_LAYERS_MAX`：急撤恢复期最大挂远层数量（>=1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_LAYER_CONVERGE_ENABLED`：急撤恢复期逐步向近层收敛。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_TEMPLATE_ENABLED`：启用恢复期“单边+低频改价”模板。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE_OFFSET_BPS`：恢复期单边偏移起点（bps）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE_OFFSET_MIN_BPS`：恢复期单边偏移最小值（bps）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_FAR_LAYER_STEP`：恢复期每阶段减少的远层层数（>=1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE_AUTO`：恢复期自动选择单边方向（基于库存与盘口偏移）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE_IMBALANCE_THRESHOLD`：触发自动单边的最小偏移阈值（0-1）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_MIN_INTERVAL_MS`：恢复期最小下单间隔（毫秒）。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_OFFSET_VOL_WEIGHT`：恢复期单边偏移随波动率放大权重。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_TEMPLATE_RESET_ENABLED`：启用模板关闭后自动恢复默认参数。
- `MM_WS_HEALTH_EMERGENCY_RECOVERY_SINGLE_SIDE_LOSS_WEIGHT`：恢复期亏损时自动单边更激进的权重（0-1）。
...
- `MM_PROTECTIVE_ONLY_FAR`：保护档只挂远层。
- `MM_PROTECTIVE_FORCE_SINGLE`：保护档强制单层挂单。
- `MM_PROTECTIVE_SINGLE_SIDE`：保护档固定单边方向（BUY/SELL/NONE）。
- `MM_PROTECTIVE_SINGLE_SIDE_MODE`：保护档单边模式（NORMAL/REMOTE）。
- `MM_PROTECTIVE_SINGLE_SIDE_OFFSET_BPS`：保护档单边偏移（bps）。
- `MM_PROTECTIVE_SINGLE_SIDE_AUTO`：保护档自动选择单边方向（基于库存/盘口偏移）。
- `MM_PROTECTIVE_SINGLE_SIDE_IMBALANCE_THRESHOLD`：保护档自动单边触发阈值（0-1）。
- `MM_PROTECTIVE_SIZE_SCALE`：保护档下挂单份额缩放系数（0-1）。
...
- `MM_AUTO_TUNE_CANCEL_WEIGHT`：自适应乘数对“撤单阈值”的影响强度（0-1）。
- `MM_AUTO_TUNE_REPRICE_WEIGHT`：自适应乘数对“重报价阈值”的影响强度（0-1）。
...
- `CROSS_PLATFORM_MAX_VWAP_DEVIATION_BPS`：跨平台 VWAP 相对盘口的最大偏离（bps），超出视为深度不足。
...
- `CROSS_PLATFORM_SHADOW_IMPACT_BPS`：影子执行固定冲击缓冲（bps）。
- `CROSS_PLATFORM_SHADOW_IMPACT_PER_LEVEL_BPS`：影子执行每档深度的冲击缓冲（bps）。
...
- `CROSS_PLATFORM_MIN_TOP_DEPTH_SHARES`：顶层最小份额（防止顶层过薄）。
- `CROSS_PLATFORM_MIN_TOP_DEPTH_USD`：顶层最小 USD 深度。
- `CROSS_PLATFORM_TOP_DEPTH_USAGE`：顶层深度使用比例（0-1，用于限制推荐下单量）。
...
- `CROSS_PLATFORM_FAILURE_RATE_WINDOW_MS`：失败率统计窗口（毫秒）。
- `CROSS_PLATFORM_FAILURE_RATE_MIN_ATTEMPTS`：统计窗口内最小尝试次数。
- `CROSS_PLATFORM_FAILURE_RATE_THRESHOLD`：触发动态收紧的失败率阈值（百分比）。
- `CROSS_PLATFORM_FAILURE_RATE_TIGHTEN_MAX`：失败率动态收紧最大倍数（>=1）。
- `CROSS_PLATFORM_FAILURE_RATE_STABILITY_SAMPLES_ADD`：失败率触发时额外稳定采样次数。
- `CROSS_PLATFORM_FAILURE_RATE_STABILITY_INTERVAL_ADD_MS`：失败率触发时额外稳定采样间隔（毫秒）。
- `CROSS_PLATFORM_FAILURE_RATE_STABILITY_MAX_SAMPLES`：失败率触发时稳定采样最大次数。
- `CROSS_PLATFORM_FAILURE_RATE_STABILITY_MAX_INTERVAL_MS`：失败率触发时稳定采样最大间隔（毫秒）。
