...
按场馆准备：
- Predict：`API_KEY`、`PRIVATE_KEY`（`JWT_TOKEN` 仅 `ENABLE_TRADING=true` 实盘时必填）
- Probable：`PROBABLE_PRIVATE_KEY`（可选兼容 `PRIVATE_KEY`）
...
复制模板并修改：
- `cp .env.example .env`
- `npm run template:predict`（Predict 做市）
- `npm run template:probable`（Probable 做市）

推荐市场并写入配置：
- `npm run market:recommend`
- `npm run market:apply`（把推荐结果写入 `MARKET_TOKEN_IDS`）

模板会自动写入官方默认 API/WS 地址（无需手填）：
- Predict API：`https://api.predict.fun`
- Predict WS：`wss://ws.predict.fun/ws`
- Probable Market API：`https://market-api.probable.markets/public/api/v1/markets/`
- Probable Orderbook API：`https://api.probable.markets/public/api/v1/book`
- Probable WS：`wss://ws.probable.markets/public/api/v1`

最小可运行配置：
- Predict：`API_KEY=...`、`PRIVATE_KEY=...`
- Probable：`PROBABLE_PRIVATE_KEY=...`、`MM_VENUE=probable`
