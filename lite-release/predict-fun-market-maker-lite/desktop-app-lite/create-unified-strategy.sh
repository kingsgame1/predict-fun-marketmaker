...
# 2. 创建统一策略文件
cat > src/strategies/unified-strategy.ts << 'EOF'
/**
...
export class UnifiedStrategy {
  ...
  analyze(market: Market, position: Position, yesPrice: number): {
    ...
    return {
      state, shouldBuy, shouldSell,
      buySize: shouldBuy ? baseSize : 0,
      sellSize: shouldSell ? Math.min(baseSize, Math.floor(total / 2)) : 0,
    };
  }

  suggestPrices(yesPrice: number, bestBid?: number, bestAsk?: number): { bid: number; ask: number } {
  ...
  updatePendingOrders(tokenId: string, buyShares: number, buyPrice: number, sellShares: number, sellPrice: number): void {
    ...
    s.pendingBuyShares = buyShares; s.pendingBuyPrice = buyPrice;
    s.pendingSellShares = sellShares; s.pendingSellPrice = sellPrice;
  }
...
echo "✅ 创建了 src/strategies/unified-strategy.ts"
