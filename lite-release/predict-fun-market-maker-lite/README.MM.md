...
第二档 YES 买单被成交 q 数量:
┌──────────────────────────────────────┐
│ 1. 检测成交: YES 买单成交 50 股       │
│ 2. 不撤单: 保留剩余挂单继续排队       │
│ 3. 立即对冲: 市价买入 NO 50 股        │
│ 4. 状态更新: hedgedShares += 50      │
└──────────────────────────────────────┘
```
...
如需自定义策略参数，可在 `.env` 配置区修改：

```env
# 统一策略参数
UNIFIED_STRATEGY_MIN_SIZE=10          # 最小订单股数
UNIFIED_STRATEGY_MAX_SIZE=500         # 最大订单股数
UNIFIED_STRATEGY_BUY_OFFSET_BPS=100   # 买单价偏移 (100bps = 1%)
UNIFIED_STRATEGY_SELL_OFFSET_BPS=100  # 卖单价偏移 (100bps = 1%)
UNIFIED_STRATEGY_HEDGE_SLIPPAGE_BPS=250 # 对冲滑点容忍
```
...
### v1.6.5
- 新增: 统一做市策略 (二档追踪 + 异步对冲 + 双轨并行)
- 新增: 桌面端 "获取 JWT Token" 按钮
- 优化: 模板默认启用交易和统一策略
- 优化: 市场推荐流动性评估
